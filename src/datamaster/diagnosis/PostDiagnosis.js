...
const ENDPOINTS = {
  create_diagnosis: 'http://18.142.73.75:5000/api/v3/datamaster/diagnosis',
};
...
function generateUniqueCode() {
    return `diag-${Math.floor(Math.random() * 100000)}`;
  }

// Request Body 
export function postDIAGNOSIS() {
  const uniqueCode = generateUniqueCode();
  const post_body = { 
      "code": uniqueCode,
      "name": "Diagnosis Name 07",
      "status": true
  };

  const regisUrl = ENDPOINTS.create_diagnosis;
