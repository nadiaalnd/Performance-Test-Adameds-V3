...
const ENDPOINTS = {
  create_general_consent: 'http://18.142.73.75:5000/api/v3/datamaster/general-consent',
};
...
function generateUniqueCode() {
    return `gc-${Math.floor(Math.random() * 100000)}`;
  }

// Request Body 
export function postGeneralConsent() {
  const uniqueCode = generateUniqueCode();
  const post_body = { 
      "code": uniqueCode,
      "name": "general consent 07",
      "isi_surat": "ini adalah isi surat general consent",
      "status": true
  };

  const regisUrl = ENDPOINTS.create_general_consent;
